...
try {
    //Load Images
    $image1 = \HC\Image::load('PNG_transparency_demonstration_1.png');
    $image2 = \HC\Image::load('PNG_transparency_demonstration_2.png');
    //$image3 = clone $image1;//-> new Image($image1, true)
    $image3 = new Image($image1, false);
    
    //merge non transparent image to non transparent
    $image2->merge($image1, 200, 200);
    $image2->save('tmp/trans1');

    //resize, trim, and paste over transparent image
    $image1->resize(200, 200);
    $image1->save('tmp/trans2');
    $image1->trim();
    $image1->getCanvas()->filledRectangle(20, 20, 50, 50, 0x50ff0000);
    $image1->save('tmp/trans3');
    unset($image1);
    ...
    $image3->save('tmp/trans4',false,9);
} catch (Exception $exc) {
